...
const ICONS = {
  home: "M3 10.5L12 3l9 7.5V21a.75.75 0 01-.75.75H15a.75.75 0 01-.75-.75v-4.5h-4.5V21A.75.75 0 019 21.75H3.75A.75.75 0 013 21v-10.5z",
};
...
type AppLayoutProps = {
  ...
  noPadding?: boolean;
  /** When true, do not show the top bar or left sidebar (e.g. base/table view). */
  bare?: boolean;
};

export function AppLayout({ children, noPadding, bare }: AppLayoutProps) {
  ...
  const isBaseRoute = !!baseId && router.pathname.startsWith("/bases/");

  const { data: base } = trpc.base.getById.useQuery(
    { id: baseId! },
    { enabled: !!baseId && isBaseRoute && !bare }
  );
  const { data: tables = [] } = trpc.table.listByBaseId.useQuery(
    { baseId: baseId! },
    { enabled: !!baseId && !!base && isBaseRoute && !bare }
  );

  if (bare) {
    return (
      <div className={styles.wrapperBare}>
        {children}
      </div>
    );
  }
  ...
  return (
    <div className={styles.wrapper}>
      <header className={styles.topBar}>
        <Link href="/" className={styles.logo} aria-label="Airtable home">
      ...
      <div className={styles.layoutRow}>
        <aside className={styles.sidebar}>
          <nav className={styles.sidebarNav}>
            <Link
              href="/"
              className={`${styles.sidebarLink} ${!isBaseRoute ? styles.sidebarLinkActive : ""}`}
            >
              <SidebarIcon d={ICONS.home} />
              <span>Home</span>
            </Link>
            {isBaseRoute && base && (
              <div className={styles.sidebarBaseSection}>
                <div className={styles.sidebarBaseHeader}>
                  <span
                    className={styles.sidebarBaseIcon}
                    style={{ background: getBaseColor(base.name) }}
                  />
                  <span className={styles.sidebarBaseName}>{base.name}</span>
                  <span className={styles.sidebarChevron} aria-hidden>▼</span>
                </div>
                <div className={styles.sidebarTableList}>
                  {tables.map((t) => (
                    <Link
                      ...
                      className={`${styles.sidebarTableLink} ${t.id === tableId ? styles.sidebarTableLinkActive : ""}`}
                    >
                      {t.name}
                    </Link>
                  ))}
                  <span className={styles.sidebarAddTable}>+ Add or import</span>
                </div>
              </div>
            )}
          </nav>
